	...
	public static Vehicle createVehicle(String description){
        ...
        if(str!=null) {
            for(int i=0; i<str.length; i++) {
                String[] attrPair = str[i].split(":");
                if(attrPair!=null && attrPair.length!=0 && attrPair.length<=2) {
                    if("type".equalsIgnoreCase(attrPair[0].trim())) {
                        type=attrPair[1].trim();
                    } else if("name".equalsIgnoreCase(attrPair[0].trim())) {
                        name=attrPair[1].trim();
                    } else if("due".equalsIgnoreCase(attrPair[0].trim())) {
                        dueAtSigning=Double.parseDouble(attrPair[1].trim());
                    } else if("length".equalsIgnoreCase(attrPair[0].trim())) {
                        leaseLength=Integer.parseInt(attrPair[1].trim());
                    } else if("monthly".equalsIgnoreCase(attrPair[0].trim())) {
                        monthlyCost=Double.parseDouble(attrPair[1].trim());
                    } else if("mile/unit".equalsIgnoreCase(attrPair[0].trim())) {
                        usage=Double.parseDouble(attrPair[1].trim());
                    } else if("allowance".equalsIgnoreCase(attrPair[0].trim())) {
                        mileageAllowance=Integer.parseInt(attrPair[1].trim());
                    } else if("charger".equalsIgnoreCase(attrPair[0].trim())) {
                        charger=Double.parseDouble(attrPair[1].trim());
                    } else {
                        System.out.println("Unmatched Attribute");
                    }
                }
            }
            lease = new Lease(dueAtSigning, leaseLength, monthlyCost, mileageAllowance);
            if("gas".equalsIgnoreCase(type)) {
                fuel = new Fuel(usage);
            } else if("electric".equalsIgnoreCase(type)) {
                fuel = new Fuel(usage, charger);
            }
            v = new Vehicle(name, fuel, lease);
        }
        return v;
    }
