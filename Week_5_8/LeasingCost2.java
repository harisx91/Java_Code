public class LeasingCost2 {
	...
	public static Vehicle createVehicle(String description){
        ...
        int beginIndex = description.indexOf("name");
        int endIndex = -1;
       
        String newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");

        String name = newString.substring(5, endIndex);
        beginIndex = description.indexOf("type");
        newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");

        String type = newString.substring(5, endIndex);
        beginIndex = description.indexOf("mile/unit");
        newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");

        String usage = newString.substring(10, endIndex);
        Fuel fuel = null;

        if (type.equalsIgnoreCase("gas")) {
            fuel = new Fuel(Double.valueOf(usage));
        } else if (type.equalsIgnoreCase("electric")) {
            beginIndex = description.indexOf("charger");
            newString = description.substring(beginIndex);
            endIndex = newString.indexOf(";");
            String charger = newString.substring(8, endIndex);
            fuel = new Fuel(Double.valueOf(usage), Double.valueOf(charger));
        }
        beginIndex = description.indexOf("due");
        newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");

        String due = newString.substring(4, endIndex);
        beginIndex = description.indexOf("length");
        newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");

        String length = newString.substring(7, endIndex);
        beginIndex = description.indexOf("monthly");
        newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");

        String monthly = newString.substring(8, endIndex);
        beginIndex = description.indexOf("allowance");
        newString = description.substring(beginIndex);
        endIndex = newString.indexOf(";");
        String allowance = newString.substring(10, endIndex);
        Lease lease = new Lease(Double.valueOf(due), Integer.valueOf(length),
            Double.valueOf(monthly),
            Integer.valueOf(allowance));
	...
	public static void computeCO2EmissionsAndCost( Vehicle[] vehicles, double gasPrice, double electricityPrice ){
        ...
        
        {
            if (vehicles[i].getFuel().getType() == Fuel.GAS)
                vehicles[i].setCO2Emission(computeCO2(vehicles[i].getLease().getMonthlyCost(),
            ...
            if (vehicles[i].getFuel().getType() == Fuel.GAS) 
                vehicles[i].setFuelCost(computeFuelCost(vehicles[i].getLease().getMonthlyCost(),
                    vehicles[i].getFuel().getUsage(),
                    vehicles[i].getLease().getMileageAllowance(),
                    gasPrice));
            else if (vehicles[i].getFuel().getType() == Fuel.ELECTRIC) 
                vehicles[i].setFuelCost(computeFuelCost(vehicles[i].getLease().getMonthlyCost(),
                    ...
                    vehicles[i].getLease().getMileageAllowance(),
                    electricityPrice));
            vehicles[i].setTotalCost(computeLeaseCost(vehicles[i].getLease().getDueAtSigning(),
                vehicles[i].getLease().getLeaseLength(),
